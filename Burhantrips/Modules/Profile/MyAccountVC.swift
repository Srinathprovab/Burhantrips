...
class MyAccountVC: BaseTableVC, ProfileDetailsViewModelDelegate {
    ...
    func getProfileDetails(response: ProfileDetailsModel) {
        ...
        DispatchQueue.main.async {[self] in
            ...
            nav.titlelbl.textColor = HexColor("#FFFFFF")
            nav.titlelbl.font = .PoppinsMedium(size: 20)
    ...
    func setupUI() {
        ...
        nav.titlelbl.font = .PoppinsMedium(size: 20)
        nav.titlelbl.text = "My Profile"
        nav.contentView.backgroundColor = .AppGreenColor
    ...
    func setupTV() {
        ...
        nav.filterBtnView.backgroundColor = .AppYellowColor
        nav.filterImg.image = UIImage(named: "pencilEdit")?.withRenderingMode(.alwaysOriginal).withTintColor(.WhiteColor)
