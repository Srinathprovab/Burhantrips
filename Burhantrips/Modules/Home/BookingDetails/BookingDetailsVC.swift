...
//    var countryCode = String()
    ...
    func setupUI() {
        ...
        sessionTimerView.addCornerRadiusWithShadow(color: .clear, borderColor: .AppGreenColor, cornerRadius: 5)
    ...
    func setupTV() {
        ...
        tablerow.append(TableRow(title:self.mbRefundable,
                                 subTitle: grand_total_Price,
                                 buttonTitle: currencyType,
                                 moreData: mbSummery,
        ...
        tablerow.append(TableRow(moreData: services, cellType:.AddonTableViewCell))
 ...
 //   MARK: - did Tap On T&C Action
    ...
    func mobileprocesspassengerDetails(response: MBPModel) {
        ...
        DispatchQueue.main.async {
            BASE_URL = ""
            self.viewmodel1?.Call_mobile_secure_booking_API(dictParam: [:], url: "https://provabdevelopment.com/burhan_trips/pro_new/mobile/index.php/flight/mobile_secure_booking/\(self.tmpFlightPreBookingId)/\(defaults.string(forKey: UserDefaultsKeys.searchid) ?? "")")
        }
    }
    ...
    func mobilePreBookingModelDetails(response: MobilePreBookingModel) {
        ...
        if response.status == false {
            ...
            mbviewmodel?.Call_mobile_pre_payment_confirmation_API(dictParam: payload, url: "https://provabdevelopment.com/burhan_trips/pro_new/mobile/index.php/flight/mobile_pre_payment_confirmation")
        }
    }
...
//        priceDetailsArry = response.pre_booking_params?.priceDetails
