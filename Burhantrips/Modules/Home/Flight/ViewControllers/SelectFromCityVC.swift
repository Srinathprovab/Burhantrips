...
class SelectFromCityVC: BaseTableVC, SelectCityViewModelProtocal {
    ...
    var filtered1:[HotelCityListModel] = []
    var cityList1:[HotelCityListModel] = []
    var keyStr = String()
    ...
    func setupUI() {
        dismissalView.layer.cornerRadius = dismissalView.layer.frame.width / 2
        ...
        titleLabel.text = "\(titleStr) Location"
        setupViews(v: searchTextfieldHolderView, radius: 6, color: .WhiteColor.withAlphaComponent(0.5))
        searchImg.image = UIImage(named: "search")?.withRenderingMode(.alwaysOriginal).withTintColor(.AppBorderColor)
        ...
        commonTableView.register(UINib(nibName: "NewFromCityTVCell", bundle: nil), forCellReuseIdentifier: "cell")
...
extension SelectFromCityVC {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! NewFromCityTVCell
        ...
        if keyStr == "hotel" {
            ...
            if(isSearchBool == true){
                ...
                cell.setAttributedString(str1: dict.name ?? "", str2: " \(dict.code ?? "")")
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let cell = tableView.cellForRow(at: indexPath) as? NewFromCityTVCell {
