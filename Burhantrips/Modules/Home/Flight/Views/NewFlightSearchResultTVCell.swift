...
class NewFlightSearchResultTVCell: TableViewCell {
    ...
    @IBOutlet weak var similarView: BorderedView!
    ...
    override func updateUI() {
        ...
        setAttributedTextnew(str1: defaults.string(forKey:UserDefaultsKeys.selectedCurrency) ?? "KWD",
                             ...
                             lbl: pricelbl,
                             str1font: UIFont.PoppinsMedium(size: 12),
        ...
        if let similarList1 = cellInfo?.data as? [[J_flight_list]] {
            ...
            if similarListCount > 1 {
                setuplabels(lbl: moreSimlarOptionlbl, text: "Same Price \(similarListCount)", textcolor: UIColor.AppLabelColor, font: UIFont.InterRegular(size: 12), align: .right)
                showSimilarlbl()
            } else {
                hideSimilarlbl()
            }
    ...
    func setupUI() {
        ...
        bottomView.layer.borderColor = HexColor("#DADADA").cgColor
...
extension NewFlightSearchResultTVCell: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as? AddFlightInfoTVCell {
            ...
            cell.airlinelbl.text = "\(data.operator_name ?? "") (\(data.operator_code ?? "")-\(data.flight_number ?? ""))"
            ...
            cell.fromCitylbl.text = "\(data.origin?.loc ?? "")"
            cell.toTimelbl.text = data.destination?.time ?? ""
            cell.toCitylbl.text = "\(data.destination?.loc ?? "")"
...
extension NewFlightSearchResultTVCell {
    ...
    func setAttributedString1(str1:String,str2:String) {
        ...
        let atter1 = [NSAttributedString.Key.foregroundColor: UIColor.AppGreenColor ,NSAttributedString.Key.font:UIFont.latoBold(size: 10),NSAttributedString.Key.strikethroughStyle: NSUnderlineStyle.single.rawValue] as [NSAttributedString.Key : Any]
        let atter2 = [NSAttributedString.Key.foregroundColor: UIColor.AppGreenColor,NSAttributedString.Key.font:UIFont.latoBold(size: 14),NSAttributedString.Key.strikethroughStyle: NSUnderlineStyle.single.rawValue] as [NSAttributedString.Key : Any]
