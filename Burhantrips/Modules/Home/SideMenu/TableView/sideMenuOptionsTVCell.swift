...
import UIKit
protocol sideMenuOptionsTVCellDelegate {
    func didTapOnMenuOptionBtn(cell: sideMenuOptionsTVCell)
}

class sideMenuOptionsTVCell: TableViewCell {
    ...
    var delegate: sideMenuOptionsTVCellDelegate?
    ...
    override func updateUI() {
        ...
        if cellInfo?.key == "menu" {
            ...
            sunImg.image = UIImage(named: cellInfo?.image ?? "")?.withRenderingMode(.alwaysOriginal).withTintColor(.AppGreenColor)
    ...
    func setAttributedString() {
        ...
        let atter2 = [NSAttributedString.Key.foregroundColor:UIColor.AppHolderViewColor ,NSAttributedString.Key.font:UIFont.InterRegular(size: 14)] as [NSAttributedString.Key : Any]
