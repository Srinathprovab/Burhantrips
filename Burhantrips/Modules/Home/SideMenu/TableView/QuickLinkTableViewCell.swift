    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cellInfo?.key == "links" {
            if indexPath.row == 0 {
                ...
                cell.menuOptionImg.image = UIImage(named: "flightIcon")?.withRenderingMode(.alwaysOriginal).withTintColor(HexColor("#017F7B"))
                
                return cell
            } else if indexPath.row == 2 {
                let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! SideMenuTitleTVCell
                cell.menuTitlelbl.text = "Hotel"
                cell.menuOptionImg.image = UIImage(named: "hotelIcon")?.withRenderingMode(.alwaysOriginal).withTintColor(HexColor("#017F7B"))
                return cell
            } else if indexPath.row == 3 {
                let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! SideMenuTitleTVCell
                cell.menuTitlelbl.text = "Visa"
                cell.menuOptionImg.image = UIImage(named: "visaIcon")?.withRenderingMode(.alwaysOriginal).withTintColor(HexColor("#017F7B"))
                return cell
            } else {
                let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! SideMenuTitleTVCell
                cell.menuTitlelbl.text = "Auto Payment"
                cell.menuOptionImg.image = UIImage(named: "payIcon")?.withRenderingMode(.alwaysOriginal).withTintColor(HexColor("#017F7B"))
                return cell
            }
        } else {
            if indexPath.row == 0 {
